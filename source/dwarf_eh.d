...
struct ExceptionHeader
{
    ...
    static ExceptionHeader* create(Throwable o) @nogc
    {
        ...
        if (eh.object)
        {
            ...
            void* bp = rtosbackend_heapalloc(ExceptionHeader.sizeof);
			ubyte[] bpd = cast(ubyte[])bp[0..ExceptionHeader.sizeof];
			foreach(ref byer; bpd) byer = 1;
			eh = cast(ExceptionHeader*)bp;
 ...
 */
extern(C) _Unwind_Reason_Code _d_eh_resume_unwind(_Unwind_Exception* unwindHeader, _Unwind_Context* context)
{
	return CONTINUE_UNWINDING(unwindHeader, context);
}
